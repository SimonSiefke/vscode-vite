...
export const vuePlugin: ServerPlugin = (ctx) => {
  ...
  ctx.app.use(async (ctx, next) => {
    ...
    let filePath = ctx.resolver.requestToFile(publicPath)

    // upstream plugins could've already read the file
    const descriptor = await parseSFC(ctx, ctx.root, filePath, ctx.body)
    if (!descriptor) {
      debug(`${ctx.url} - 404`)
      ctx.status = 404
      return
    ...
    if (!query.type) {
      ...
      watchFileIfOutOfRoot(ctx.watcher, ctx.root, filePath)
    ...
    if (query.type === 'style') {
      ...
      const result = await compileSFCStyle(
        ...
        filePath,
        publicPath,
        ctx.config.cssPreprocessOptions
      )
      ctx.type = 'js'
  ...
  const handleVueReload = (ctx.watcher.handleVueReload = async (
    ctx: Context,
    filePath: string,
    timestamp: number = Date.now(),
    content?: string
  ) => {
    const publicPath = ctx.resolver.fileToRequest(filePath)
    const cacheEntry = vueCache.get(filePath)
    const { send } = ctx.watcher

    debugHmr(`busting Vue cache for ${filePath}`)
    vueCache.del(filePath)

    const descriptor = await parseSFC(ctx, ctx.root, filePath, content)
    if (!descriptor) {
      // read failed
      return
    }
    ...
    const sendReload = () => {
      ...
      console.log(
        chalk.green(`[vite:hmr] `) +
          `${path.relative(ctx.root, filePath)} updated. (reload)`
      )
    }
    ...
    if (updateType.length) {
      console.log(
        chalk.green(`[vite:hmr] `) +
          `${path.relative(ctx.root, filePath)} updated. (${updateType.join(
            ' & '
          )})`
      )
    }
  })
  ctx.watcher.on('change', (file) => {
    if (file.endsWith('.vue')) {
      handleVueReload(ctx, file)
    }
  })
}
...
async function parseSFC(
  ...
  if (!content) {
    try {
      content = await ctx.read(null, filePath)
    } catch (e) {
      return
    }
  }
